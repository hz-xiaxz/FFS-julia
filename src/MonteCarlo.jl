...
function MC(params::AbstractDict)
    nx = params[:nx]
    ny = params[:ny]
    return MC{B}(BitVector(zeros(2 * nx * ny)))
end
...
function Carlo.init!(mc::MC{B}, ctx::MCContext, params::AbstractDict) where {B}
    lat = LatticeRectangular(params[:nx], params[:ny], params[:B])
    orb = AHmodel(lat, params[:t], params[:W], params[:U], params[:N_up], params[:N_down])
    conf = hcat(FFS(ctx.rng, orb.U_up), FFS(ctx.rng, orb.U_down))
    mc.model = orb
    mc.conf = conf
end
