...
@testset "FFS" begin
    ...
    @testset "Error handling" begin
        ...
        # Invalid dimensions
        @test_throws ArgumentError FFS(rng, randn(3, 4))  # L < N
        @test_throws ArgumentError FFS(rng, Matrix{Float64}(undef, 0, 0))  # Empty matrix

        # Non-normalized columns
        U = randn(4, 2)  # Non-orthogonal columns
        @test_logs((:warn, r"Probabilities don't sum to 1"),
            (:warn, r"Probabilities don't sum to 1"),
            FFS(rng, U))
    end
